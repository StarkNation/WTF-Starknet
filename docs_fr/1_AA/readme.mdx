...
Pourquoi ? Parce que votre clé privée **est** votre compte, aussi perdre votre clé signifie perdre votre compte, votre argent, vos données: Tous vos biens crypto !

Avec l'Account Abstraction (AA), si un utilisateur perd ses accès par erreur, il devient possible de l'aider à récupérer ses informations. Comment ? chaque compte est un en fait un smart-contrat (ou contrat intelligent) qui contient une logique et met en œuvre un flux.

Utiliser une logique de smart-contrat pour spécifier non seulement les effets de la transaction, mais également la logique de paiement et de validation des frais, offre de nombreux avantages de sécurité importants, tels que les portefeuilles de récupération multisignatures et intelligents, la possibilité de changer de clé sans changer de portefeuille et la sécurité quantique.
et bien plus encore.
...
* Les Externally Owned Account (EOA) ou comptes détenus en externe

*Les EOA sont des adresses de comptes utilisateurs traditionnels.*
...
## 2. Les Comptes et l'AA (Account Abstraction)

L'Account Abstraction unifie les notions de comptes de contrats et les EOA.
...
Les nouvelles expériences qui s'offrent ainsi à vous proviennent plus des fonctionnalités rendues possibles par l'abstraction de comptes que de l'abstraction elle-même.

Il ne s'agit plus maintenant d'avoir un compte unique pour tous les cas d'usage, mais avoir un compte adapté à chacun de ses besoins.
...
On considère ainsi que l'Account Abstraction vous offre une "validité de transaction programmable".
...
Parmi l'esemble des portefeuilles disponibles, nous en trouvons deux principales qui supportent l'AA - nous les détaillons ci-dessous:
...
Une fois cela effectué, vous aurez la possibilité d'envoyer, échanger et ajouter des tokens.


#### Les fonctionnalités d'Argent X

Le wallet Argent X possède toutes les fonctionnalités d'un portefeuille crypto traditionnel, mais comme il est un en fait un smart-contrat Starknet, il possède des propriétés uniques:
...
maintenant que vous savez de quoi nous parlons, explorons ! 

### 4.2 Braavos

Braavos n'est pas seulement une application pour navigateur, c'est aussi une application mobile native, vous pouvez donc stocker vos tokens, transférer, échanger et gérer vos collections NFT, mais plus encore: accéder et vous connecter à n'importe quel dApp sur Starknet depuis votre téléphone.

Votre mobile est toujours avec vous, vous permettant d'accéder à vos cryptos quand et où vous le souhaitez avec la confidentialité apportée par la biométrique.
...
2. Accrochez l'extension Braavos à votre navigateur, pour un accès rapide, et appuyez sur "Get Started"
...
6. N'oubliez pas que vous devez déposer des jetons dans votre portefeuille si vous souhaitez utiliser le réseau principal, afin d'activer votre compte (qui est en fait un smart-contrat).
